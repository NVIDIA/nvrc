...
/// Determine GPU architecture based on device name heuristics
...
/// Get GPU architecture based on device ID
fn get_gpu_architecture_by_device_id(device_id: u16, bdf: &str) -> Result<GpuArchitecture> {
    ...
    let pci_db = get_pci_ids_database().with_context(|| "Failed to get embedded PCI database")?;
...
impl NVRC {
    ...
    pub fn query_gpu_cc_mode(&mut self) -> Result<()> {
        ...
        let gpu_devices: Vec<_> = self.nvidia_devices.iter().filter(|d| matches!(d.device_type, DeviceType::Gpu)).collect();
