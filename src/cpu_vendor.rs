...
impl NVRC {
    pub fn query_cpu_vendor(&mut self) -> Result<()> {
        ...
        let cpu_vendor = BufReader::new(file)
            .lines()
            .map_while(Result::ok)
    ...
    fn detect_vendor_from_line(&self, line: &str) -> Option<String> {
        match line {
            l if l.contains(AMD_VENDOR_ID) => Some(AMD_VENDOR.to_string()),
            l if l.contains(INTEL_VENDOR_ID) => Some(INTEL_VENDOR.to_string()),
            l if l.contains("CPU implementer") && l.contains(ARM_IMPLEMENTER_ID) => {
