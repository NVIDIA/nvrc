...
pub fn classify_device_type(vendor_id: u16, device_id: u16, class_id: u32) -> Result<DeviceType> {
    ...
    if is_bridge_class(class_id) {
        ...
        debug!(
            "Unknown NVIDIA bridge device (device ID: 0x{:04x}, class: 0x{:06x})",
            device_id, class_id
        );
    ...
    debug!(
        "Unknown NVIDIA device type (device ID: 0x{:04x}, class: 0x{:06x})",
        device_id, class_id
    );
...
mod tests {
    ...
    fn test_unknown_device_handling() {
        ...
        let unknown_bridge_result =
            classify_device_type(NVIDIA_VENDOR_ID, 0x9999, class_ids::BRIDGE_OTHER);
