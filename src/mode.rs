...
//! Auto-detect NVRC mode from PCI and InfiniBand hardware topology.
//!
//! Scans `/sys/bus/pci/devices` for NVIDIA GPUs and NVSwitches, and
//! `/sys/class/infiniband` for CX7 bridge LPFs (SW_MNG marker in VPD), to
//! determine the correct operating mode and fabric manager configuration.
//!
...
pub fn detect() -> Detection {
    detect_from(PCI_DEVICES, IB_CLASS)
}

fn detect_from(pci_path: &str, ib_path: &str) -> Detection {
    let nvswitches = count_nvswitches_from(pci_path);
    let gpus = count_gpus_from(pci_path);
    let sw_mng = count_sw_mng_from(ib_path);

    debug!(
        "topology: {} GPU, {} NVSWITCH, {} IB_SW_MNG",
    ...
    match (nvswitches, gpus, sw_mng) {
        ...
        (0, 8, 2) => {
            debug!(
                "mode: gpu FABRIC_MODE=0, {} GPU + {} IB_SW_MNG",
                gpus, sw_mng
            );
            Detection {
                mode: "gpu",
        ...
        _ => {
            panic!(
                "unexpected topology: {} NVSWITCH, {} GPU, {} IB_SW_MNG — cannot determine mode",
...
/// Count NVLink management NICs (SW_MNG marker in VPD).
/// These are distinct from regular CX7 NICs used for GPUDirect RDMA.
fn count_sw_mng_from(ib_path: &str) -> usize {
    let Ok(entries) = fs::read_dir(ib_path) else {
        return 0;
    };
    entries
        .flatten()
        .filter(|e| {
            fs::read(e.path().join("device/vpd"))
...
mod tests {
    ...
    fn create_ib_device(dir: &TempDir, name: &str, vpd_content: &[u8]) {
        let dev = dir.path().join(name);
        let vpd_path = dev.join("device/vpd");
    ...
    // --- SW_MNG device counting ---

    #[test]
    fn test_count_sw_mng_single() {
        let tmpdir = TempDir::new().unwrap();
        create_ib_device(&tmpdir, "mlx5_0", b"some data SW_MNG more data");
    ...
    fn test_count_sw_mng_four() {
        let tmpdir = TempDir::new().unwrap();
        for i in 0..4 {
            create_ib_device(&tmpdir, &format!("mlx5_{}", i), b"SW_MNG");
        }
    ...
    fn test_count_sw_mng_skips_non_sw_mng() {
        let tmpdir = TempDir::new().unwrap();
        create_ib_device(&tmpdir, "mlx5_0", b"SW_MNG");
        create_ib_device(&tmpdir, "mlx5_1", b"some other data");
        assert_eq!(count_sw_mng_from(tmpdir.path().to_str().unwrap()), 1);
    }

    #[test]
    fn test_count_sw_mng_no_ib_dir() {
    ...
    fn test_detect_cpu_mode() {
        ...
        let ib = TempDir::new().unwrap();
        let d = detect_from(pci.path().to_str().unwrap(), ib.path().to_str().unwrap());
    ...
    fn test_detect_gpu_bare_metal_nvl5() {
        let pci = TempDir::new().unwrap();
        let ib = TempDir::new().unwrap();
        // 8 GPUs + 2 LPF (SW_MNG) + 2 FC PF on PCIe, no NVSwitches
        for i in 0..8 {
            create_pci_device(&pci, &format!("0000:4{}:00.0", i), "0x10de\n", "0x030200\n");
        }
        create_ib_device(&ib, "mlx5_0", b"SW_MNG");
        create_ib_device(&ib, "mlx5_1", b"SW_MNG");
        create_ib_device(&ib, "mlx5_2", b"other");
        create_ib_device(&ib, "mlx5_3", b"other");
        let d = detect_from(pci.path().to_str().unwrap(), ib.path().to_str().unwrap());
        assert_eq!(d.mode, "gpu");
    ...
    fn test_detect_servicevm_nvl5() {
        let pci = TempDir::new().unwrap();
        let ib = TempDir::new().unwrap();
        // NVL5: no NVSwitches or GPUs on PCIe, only CX7 bridges via IB
        // 2 LPF (SW_MNG) + 2 FC PF (no SW_MNG) = 4 CX7 PFs total
        create_ib_device(&ib, "mlx5_0", b"SW_MNG");
        create_ib_device(&ib, "mlx5_1", b"SW_MNG");
        create_ib_device(&ib, "mlx5_2", b"other");
        create_ib_device(&ib, "mlx5_3", b"other");
    ...
    fn test_detect_unexpected_topology_panics() {
        ...
        let result = panic::catch_unwind(|| {
            detect_from(pci.path().to_str().unwrap(), ib.path().to_str().unwrap());
        });
